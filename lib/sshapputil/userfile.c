...
/*
 * $Id: userfile.c,v 1.2 1998/07/10 13:47:22 tri Exp $
 * $Log: userfile.c,v $
 * $EndLog$
 */
  ...
  for (offset = 0; offset < len; offset += bytes)
    {
      ...
      if (bytes <= 0)
	ssh_fatal("userfile_packet_send: child has died: %s", strerror(errno));
    }
  
  len = ssh_buffer_len(&packet);
  for (offset = 0; offset < len; offset += bytes)
    {
      bytes = write(userfile_output, ssh_buffer_ptr(&packet) + offset, 
		    len - offset);
      if (bytes <= 0)
	ssh_fatal("userfile_packet_send: child has died: %s", strerror(errno));
    }
  ...
  for (offset = 0; offset < len; offset += bytes)
      ...
      if (bytes <= 0)
	{
	  if (getuid() == geteuid()) /* presumably child - be quiet */
	    exit(0);
	  ssh_fatal("userfile_read_raw: child has died: %s", strerror(errno));
	}
    }

  len = SSH_GET_32BIT(buf);
  if (len > 32000)
    ssh_fatal("userfile_read_raw: received packet too long.");
...
static void userfile_packet_read(int expected_type)
{
  ...
  if (type != expected_type)
    ssh_fatal("userfile_read_packet: unexpected packet type: got %d, expected %d",
	  type, expected_type);
}
...
static void userfile_child_server()
{
  ...
  for (;;)
	...
	case USERFILE_RENAME:
	  ...
	  userfile_packet_start(USERFILE_RENAME_REPLY);
	  buffer_put_int(&packet, ret);
	  userfile_packet_send();
	...
	case USERFILE_SHARED_LOCK:
	  ...
	  userfile_packet_start(USERFILE_SHARED_LOCK_REPLY);
	  buffer_put_int(&packet, ret);
	  userfile_packet_send();
	...
	case USERFILE_EXCL_LOCK:
	  ...
	  userfile_packet_start(USERFILE_EXCL_LOCK_REPLY);
	  buffer_put_int(&packet, ret);
	  userfile_packet_send();
	...
	case USERFILE_UNLOCK:
	  ...
	  userfile_packet_start(USERFILE_UNLOCK_REPLY);
	  buffer_put_int(&packet, ret);
	  userfile_packet_send();
	...
	default:
	  ssh_fatal("userfile_child_server: packet type %d", type);
	}
    }
}
		   ...
		   void (*cleanup_callback)(void *), void *context)
{
  ...
  if (userfile_initialized)
    ssh_fatal("userfile_init already called");
...
UserFile userfile_open(uid_t uid, const char *path, int flags, mode_t mode)
{
  ...
  if (!userfile_initialized)
    ssh_fatal("userfile_open: using non-current uid but not initialized "
	      "(uid=%d, path=%.50s)",
	      (int)uid, path);
  
  if (uid != userfile_uid)
    ssh_fatal("userfile_open: uid not current and not that of child: "
	      "uid=%d, path=%.50s",
	      (int)uid, path);
...
int userfile_close(UserFile uf)
{
  ...
  switch (uf->type)
    ...
    default:
      ssh_fatal("userfile_close: type %d", uf->type);
      /*NOTREACHED*/
      return -1;
    }
}
...
static void userfile_invalidate_buffer(UserFile uf)
{
  switch (uf->type)
    {
    case USERFILE_LOCAL:
...
static int userfile_fill(UserFile uf)
{
  ...
  if (uf->buf_first < uf->buf_last)
    ssh_fatal("userfile_fill: buffer not empty");
  ...
  switch (uf->type)
	...
	ssh_fatal("userfile_fill: got more than data than requested");
    ...
    default:
      ssh_fatal("userfile_fill: type %d", uf->type);
    }
...
int userfile_getc(UserFile uf)
{
  if (uf->buf_first >= uf->buf_last)
	...
	return -1;
      ...
      if (uf->buf_first >= uf->buf_last)
	ssh_fatal("userfile_getc/fill error");
    }
...
int userfile_write(UserFile uf, const void *buf, unsigned int len)
{
  ...
  switch (uf->type)
    ...
    default:
      ssh_fatal("userfile_write: type %d", uf->type);
      /*NOTREACHED*/
      return 0;
    }
}
...
off_t userfile_lseek(UserFile uf, off_t offset, int whence)
{
  switch (uf->type)
    ...
    default:
      ssh_fatal("userfile_lseek: type %d", uf->type);
      /*NOTREACHED*/
      return 0;
    }
}
...
int userfile_mkdir(uid_t uid, const char *path, mode_t mode)
{
  ...
  if (!userfile_initialized)
    ssh_fatal("userfile_mkdir with uid %d", (int)uid);
  
  if (uid != userfile_uid)
    ssh_fatal("userfile_mkdir with wrong uid %d", (int)uid);
...
int userfile_stat(uid_t uid, const char *path, struct stat *st)
{
  ...
  if (!userfile_initialized)
    ssh_fatal("userfile_stat with uid %d", (int)uid);
  
  if (uid != userfile_uid)
    ssh_fatal("userfile_stat with wrong uid %d", (int)uid);
...
int userfile_remove(uid_t uid, const char *path)
{
  ...
  if (!userfile_initialized)
    ssh_fatal("userfile_remove with uid %d", (int)uid);
  
  if (uid != userfile_uid)
    ssh_fatal("userfile_remove with wrong uid %d", (int)uid);
...
int userfile_rename(uid_t uid, const char *oldpath, const char *newpath)
{
  /* Perform directly if with current effective uid. */
  if (uid == geteuid())
  ...
  if (!userfile_initialized)
    ssh_fatal("userfile_rename with uid %d", (int)uid);
  
  if (uid != userfile_uid)
    ssh_fatal("userfile_rename with wrong uid %d", (int)uid);

  userfile_packet_start(USERFILE_RENAME);
  buffer_put_uint32_string(&packet, oldpath, strlen(oldpath));
  buffer_put_uint32_string(&packet, newpath, strlen(newpath));
  userfile_packet_send();

  userfile_packet_read(USERFILE_RENAME_REPLY);
  return buffer_get_int(&packet);
}

/* Performs file locking using the given uid */

int userfile_lock_shared(UserFile uf, off_t offset, off_t len)
{
  switch (uf->type)
    {
    case USERFILE_LOCAL:
    ...
    case USERFILE_REMOTE:
      userfile_packet_start(USERFILE_SHARED_LOCK);
      buffer_put_int(&packet, uf->handle);
      buffer_put_int(&packet, offset);
      buffer_put_int(&packet, len);
      userfile_packet_send();

      userfile_packet_read(USERFILE_SHARED_LOCK_REPLY);
      return buffer_get_int(&packet);

    default:
      ssh_fatal("userfile_lock_shared: type %d", uf->type);
      /*NOTREACHED*/
      return 0;
    }
}

int userfile_lock_exclusive(UserFile uf, off_t offset, off_t len)
{
  switch (uf->type)
    {
    case USERFILE_LOCAL:
    ...
    case USERFILE_REMOTE:
      userfile_packet_start(USERFILE_EXCL_LOCK);
      buffer_put_int(&packet, uf->handle);
      buffer_put_int(&packet, offset);
      buffer_put_int(&packet, len);
      userfile_packet_send();

      userfile_packet_read(USERFILE_EXCL_LOCK_REPLY);
      return buffer_get_int(&packet);

    default:
      ssh_fatal("userfile_lock_exclusive: type %d", uf->type);
      /*NOTREACHED*/
      return 0;
    }
}

int userfile_unlock(UserFile uf, off_t offset, off_t len)
{
  switch (uf->type)
    {
    case USERFILE_LOCAL:
    ...
    case USERFILE_REMOTE:
      userfile_packet_start(USERFILE_UNLOCK);
      buffer_put_int(&packet, uf->handle);
      buffer_put_int(&packet, offset);
      buffer_put_int(&packet, len);
      userfile_packet_send();

      userfile_packet_read(USERFILE_UNLOCK_REPLY);
      return buffer_get_int(&packet);

    default:
      ssh_fatal("userfile_unlock: type %d", uf->type);
      /*NOTREACHED*/
      return 0;
    }
}
...
UserFile userfile_popen(uid_t uid, const char *command, const char *type)
{
  ...
  if (!userfile_initialized)
    ssh_fatal("userfile_popen: using non-current uid but not initialized (uid=%d)",
	  (int)uid);
  
  if (uid != userfile_uid)
    ssh_fatal("userfile_popen: uid not current and not that of child: uid=%d",
	  (int)uid);
...
char *userfile_get_des_1_magic_phrase(uid_t uid)
{
  ...
  if (!userfile_initialized)
    ssh_fatal("userfile_get_des_1_magic_phrase with uid %d", (int)uid);
  
  if (uid != userfile_uid)
    ssh_fatal("userfile_get_des_1_magic_phrase with wrong uid %d", (int)uid);
